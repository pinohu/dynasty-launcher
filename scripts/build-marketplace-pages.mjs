...
<section class="section"><h2>What ships behind the promise</h2><p>This marketplace page is not standalone brochure copy. Every activated item belongs to the same business-unit standard: catalog traceability, tenant entitlement checks, capability checks, event logs, workflow state, internal provider interfaces, optional PostHog analytics, AI/MCP tool metadata, central configuration, build repair telemetry, and resumable checkpoints.</p><div class="grid"><article class="card"><h3>Traceable activation</h3><p>Each promise maps back to catalog data, API records, activation routes, capability requirements, and deferred states when a tenant is not ready.</p></article><article class="card"><h3>Self-hostable core</h3><p>Core logic uses internal interfaces for payments, email, storage, analytics, documents, AI, signatures, and notifications. External services stay optional adapters.</p></article><article class="card"><h3>AI and MCP ready</h3><p>Generated builds include tool-registry evidence, permissions, workflow definitions, prompt assets, retrieval scaffolding, and model routing guidance.</p></article><article class="card"><h3>Repairable build pipeline</h3><p>Failed build classes feed into detector/remediator pairs, BUILD-REPORT telemetry, and resume checkpoints so future builds continue from the last verified state.</p></article><article class="card"><h3>Media and analytics expansion</h3><p>When enabled, video asset planning and PostHog-ready event tracking connect marketing content to product, funnel, revenue, and customer lifecycle metrics.</p></article><article class="card"><h3>Niche validation</h3><p>Content, offers, workflows, pricing, contracts, and support language are checked against the niche, buyer, location, business model, and primary offer.</p></article></div></section>`;
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<section class="section"><h2>Link back to the main site</h2><p>Use the main Your Deputy site to compare the launch standard, browse marketplace elements, inspect the maturity matrix, review deliverables, and choose the right autonomous business-unit path.</p><div class="cta-row" style="margin-top:18px"><a class="btn btn-primary" href="/">Return to Your Deputy</a><a class="btn btn-ghost" href="/marketplace">Explore marketplace</a><a class="btn btn-ghost" href="/maturity">See maturity</a></div></section>`;
}
