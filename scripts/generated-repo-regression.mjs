...
const semanticCorpus = Object.entries(repaired.files)
  .filter(([file]) => /^(sdlc|pre-saas|gtm|customer-success|operations|experiments|memory|ux|security|governance|mcp|observability|deployment|architecture|evals|analytics|content\/video|builder-governance|code-context|context-optimization|agent-runtime|media\/voice)\//.test(file))
  .map(([, body]) => body)
  .join('\n')
  .toLowerCase();
for (const marker of ['tam', 'sam', 'som', 'prd', 'brd', 'rice', 'kano', 'uat', 'cac', 'ltv', 'a/b testing', 'revenue reinvestment', 'winner detection', 'saas transition', 'posthog', 'session replay', 'feature flags', 'experiments', 'surveys', 'karpathy', 'wrong assumptions', 'surgical changes', 'goal-driven execution', 'claude-context', 'semantic code search', 'incremental indexing', 'ast-aware chunking', 'hermes agent', 'persistent memory', 'skill creation', 'resume from failure', 'vibevoice', 'voiceover', 'transcription', 'deepfake prevention', 'rtk', 'token compression', 'context efficiency', 'owasp asvs', 'owasp samm', 'nist ai rmf', 'model context protocol', 'opentelemetry', 'github actions', 'tenant isolation', 'video asset pipeline', 'launch video', 'video-use', 'ffmpeg', 'scaffold fallback']) {
